...
const assertEqual = function(actual, expected) {//included as required
  if (actual === expected) {
...
const eqObjects = function(object1, object2) {
  if (Object.keys(object1).length !== Object.keys(object2).length) {//create arrays with objects using Objects.keys
    return false;//compare array lengths to see if the arrays are not equal(if not, stop and return false)
  }
  for (const key of Object.keys(object1)) {//check through one of the objects using a for...of loop as the keys are arrays
    if (Array.isArray(object1[key]) && Array.isArray(object2[key])) {//make sure that the two values are arrays and then pass them to eqArrays if so
      if (!eqArrays(object1[key], object2[key])) {//check using negation of eqArrays to confirm they are arrays
